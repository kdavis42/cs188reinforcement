...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        route = util.flipCoin(self.epsilon)
        if route:
    ...
    def update(self, state, action, nextState, reward):
        ...
        newQVal = (1 - self.alpha)*oldQ + self.alpha*sample
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        diff = sample - oldQ
        addiWeight = self.alpha*diff
        for key in features:
          self.weights[key] = self.weights[key] + addiWeight*features[key]
